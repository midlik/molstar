...
function defaultCanvas3DParams(): Partial<Canvas3DProps> {
    return {
        camera: {
            mode: 'orthographic',
            helper: {
...
export class ImageRenderer {
    ...
    constructor(private canvasSize: { width: number, height: number }) {
        this.webgl = createContext(createGLContext(this.canvasSize.width, this.canvasSize.height, {
            antialias: true,
        ...
        this.canvas3d = Canvas3D.create({ webgl: this.webgl, input, passes, attribs } as Canvas3DContext, defaultCanvas3DParams());
