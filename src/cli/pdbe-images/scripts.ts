...
async function createStructureComponent(plugin: PluginContext, structure: StateObjectSelector, params: Partial<StructureComponentParams>):
    Promise<StateObjectSelector<PluginStateObject.Molecule.Structure, any> | null> {
...
function adjustCamera(plugin: PluginContext, change: (s: Camera.Snapshot) => Camera.Snapshot) {
    ...
    // plugin.canvas3d.commit(true); // this one is proooooobably not needed
