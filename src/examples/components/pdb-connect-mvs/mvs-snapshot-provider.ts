...
import { MolComponents } from '..';
import { GlobalMetadata, MVSData, MVSData_State, MVSData_States, Snapshot } from '../../../extensions/mvs/mvs-data';
import * as Builder from '../../../extensions/mvs/tree/mvs/mvs-builder';
...
const SnapshotKinds = ['entry', 'assembly', 'entity'] as const satisfies SnapshotKind[];

type SnapshotSpec<TKind extends SnapshotKind = SnapshotKind> =
...
export class MVSSnapshotProvider {
    ...
    listSnapshotKinds(): SnapshotKind[] {
    ...
    async getSnapshot(spec: SnapshotSpec): Promise<MVSData_State> {
        ...
        const description = [
            `### ${spec.name}`,
            `**${spec.kind}** ${JSON.stringify(spec.params)}`,
            '---',
        ];
        switch (spec.kind) {
            case 'entry':
...
export async function foo(pdbId: string = '1tqn') {
    const mc = new MolComponents();
    const ctx: MolComponentContext = mc.getContext();

    async function loadSpec(spec: SnapshotSpec) {
        const snapshot = await provider.getSnapshot(spec);
        // const snapshot = mvsSingleStateToMultipleState(snapshot_); // This conversion is just to show markdown description in current version of <mc-snapshot-markdown>
        console.log(MVSData.toPrettyString(snapshot))
        ctx.dispatch({
            kind: 'load-mvs',
            data: snapshot,
        });
    ...
    const provider = mc.getDefaultMVSSnapshotProvider();
    console.log('provider', provider);
    const specs = await provider.listSnapshots(pdbId);
    console.log('SNAPSHOT SPECS:');
    for (const spec of specs) {
        console.log(JSON.stringify(spec));
        const btn = document.createElement('button');
        btn.textContent = spec.name;
        btn.classList.add('mvs-button');
        btn.addEventListener('click', () => {
            loadSpec(spec);
            for (const other of Array.from(document.getElementById('controls')!.getElementsByClassName('mvs-button'))) {
                other.classList.remove('button-primary');
            }
...
function mvsSingleStateToMultipleState(state: MVSData_State): MVSData_States {
    ...
    const metadata = GlobalMetadata.create({
        title: state.metadata.title,
        description: state.metadata.description,
        description_format: state.metadata.description_format
    });
    return MVSData.createMultistate([snapshot], metadata);
}
