...
import { defaultCanvas3DParams, ImageRendererOptions, STYLIZED_POSTPROCESSING } from '../../mol-canvas3d/renderer';
import { HeadlessPluginContext } from '../../mol-plugin/headless-plugin-context';
import { DefaultPluginSpec } from '../../mol-plugin/spec';
import { PDBeAPI } from './api';

import { NaughtySaver } from './helpers';
import { ImageGenerator } from './scripts';
...
async function tryPlugin(args: Args) {
    ...
    for (let i = 0; i < 1; i++) {
        ...
        const imageGenerator = new ImageGenerator(plugin, name => saver.save(name), api);
        await imageGenerator.processUrl(localUrl, args.pdbid);
